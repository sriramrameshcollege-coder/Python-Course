...
def get_stats(transactions: list[Transaction]) -> dict[str, float]:
    ...
    return {
        "income_mean": float(np.mean(income_amounts))
        if income_amounts.size > 0
        else 0.0,
        "income_std": float(np.std(income_amounts)) if income_amounts.size > 0 else 0.0,
        "expense_mean": float(np.mean(expense_amounts))
        if expense_amounts.size > 0
        else 0.0,
        "expense_std": float(np.std(expense_amounts))
        if expense_amounts.size > 0
        else 0.0,
        "biggest_income": float(np.max(income_amounts))
        if income_amounts.size > 0
        else 0.0,
        "biggest_expense": float(np.min(expense_amounts))
        if expense_amounts.size > 0
        else 0.0,
    }
...
def get_biggest_expense_category(
